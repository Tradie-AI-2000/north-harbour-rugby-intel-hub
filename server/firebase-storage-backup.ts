...
async function createTrainingSession(sampleRecord: FirebaseGPSRecord): Promise<void> {
  try {
    const sessionRef = db.collection(COLLECTIONS.TRAINING_SESSIONS).doc(sampleRecord.sessionId);
    
    const sessionData = {
      sessionId: sampleRecord.sessionId,
      sessionName: sampleRecord.sessionName,
      sessionDate: new Date(sampleRecord.sessionDate).toISOString(),
    ...
    console.log(`✅ Training session document created: ${sampleRecord.sessionId}`);
